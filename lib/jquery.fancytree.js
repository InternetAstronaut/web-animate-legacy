...
;(($, window, document, undefined) => {
    ...
    let // initialized below
    FT = null;

    const // strings are considered image urls if they conatin '.' or '/'
    TEST_IMG = new RegExp(/\.|\//);

    const REX_HTML = /[&<>"'\/]/g;
    const REX_TOOLTIP = /[<>"'\/]/g;
    const RECURSIVE_REQUEST_ERROR = "$recursive_request";
    const ENTITY_MAP = {"&": "&amp;", "<": "&lt;", ">": "&gt;", "\"": "&quot;", "'": "&#39;", "/": "&#x2F;"};
    const IGNORE_KEYCODES = { 16: true, 17: true, 18: true };
    ...
    const MOUSE_BUTTONS = { 0: "", 1: "left", 2: "middle", 3: "right" };

    const // Boolean attributes that can be set with equivalent class names in the LI tags
    CLASS_ATTRS = "active expanded focus folder hideCheckbox lazy selected unselectable".split(" ");

    const CLASS_ATTR_MAP = {};

    const // Top-level Fancytree node attributes, that can be set by dict
    ...
    const // Mapping of lowercase -> real name (because HTML5 data-... attribute only supports lowercase)
    NODE_ATTR_LOWERCASE_MAP = {};

    const // Attribute names that should NOT be added to node.data
    ...
    function consoleApply(method, args){
        let i;
        let s;
        const fn = window.console ? window.console[method] : null;
    ...
    function _isNode({tree, statusNodeType}) {
        return !!(tree && statusNodeType !== undefined);
    }
    ...
    function isVersionAtLeast(dottedVersion, major, minor, patch){
        ...
        let v;
        let t;
        const verParts = $.map($.trim(dottedVersion).split("."), e => { return parseInt(e, 10); });
        const testParts = $.map(Array.prototype.slice.call(arguments, 1), e => { return parseInt(e, 10); });
    ...
    function _makeVirtualFunction(methodName, tree, base, extension, extName){
        ...
        const proxy = (() => {
            const // org. tree method or prev. proxy
            prevFunc = tree[methodName];

            const //
            baseFunc = extension[methodName];

            const _local = tree.ext[extName];

            const _super = (...args) => {
				return prevFunc.apply(tree, args);
			};

            const _superApply = args => {
				return prevFunc.apply(tree, args);
			};

            // Return the wrapper function
            return (...args) => {
                const prevLocal = tree._local;
                const prevSuper = tree._super;
                const prevSuperApply = tree._superApply;

                try{
                    tree._local = _local;
                    tree._super = _super;
                    tree._superApply = _superApply;
                    return baseFunc.apply(tree, args);
                }finally{
    ...
    function _escapeTooltip(s){
        return (`${s}`).replace(REX_TOOLTIP, s => {
            return ENTITY_MAP[s];
        });
    }
    ...
    class FancytreeNode {
        ...
        addChildren(children, insertBefore) {
            ...
            let l;
            let pos;
            const origFirstChild = this.getFirstChild();
            const origLastChild = this.getLastChild();
            let firstNode = null;
            const nodeList = [];
            ...
            if(insertBefore == null){
                ...
                this.children.splice(...[pos, 0].concat(nodeList));
            }
        ...
        addNode(node, mode) {
            ...
            case "firstChild":
                ...
                const insertBefore = (this.children ? this.children[0] : null);
        ...
        applyPatch(patch) {
            ...
            let name; // TODO: should be global

            let promise;
            let v;
            const IGNORE_MAP = { children: true, expanded: true, parent: true };
         ...
         */
        discardMarkup(includeSelf) {
            const fn = includeSelf ? "nodeRemoveMarkup" : "nodeRemoveChildMarkup";
        ...
        _changeSelectStatusAttrs(state) {
            let changed = false;
    ...
    */
    ...
    //		return this.parent.children.indexOf(this);
         ...
         * @returns {string}
         */
        getIndexHier(separator = ".", digits) {
            let s;
            const res = [];
            $.each(this.getParentList(false, true), (i, o) => {
                s = `${o.getIndex() + 1}`;
                if( digits ){
         ...
         * @returns {string}
         */
        getKeyPath(excludeSelf) {
            const path = [];
            const sep = this.tree.options.keyPathSeparator;
            this.visitParents(n => {
                if(n.parent){
                    path.unshift(n.key);
                }
        ...
        makeVisible(opts) {
            ...
            const effects = !(opts && opts.noAnimation === true);
            const scroll = !(opts && opts.scrollIntoView === false);
            ...
            $.when(...deferreds).done(() => {
                ...
                if( scroll ){
                    that.scrollIntoView(effects).done(() => {
        ...
        moveTo(targetNode, mode, map) {
            ...
            const targetParent = (mode === "child") ? targetNode : targetNode.parent;
    ...
    //			_assert(false, "Cross-tree move is not yet implemented.");
        ...
        navigate(where, activate) {
            ...
            switch( where ) {
                case KC.BACKSPACE:
                    ...
                    break;
                case KC.HOME:
                    this.tree.visit(n => {  // goto first visible node
                        if( $(n.span).is(":visible") ) {
                            res = _goto(n);
                            return false;
                        }
                    ...
                    break;
                case KC.END:
                    this.tree.visit(n => {  // goto last visible node
                        if( $(n.span).is(":visible") ) {
                            res = n;
                        }
                    });
        ...
        replaceWith(source) {
            ...
            const pos = $.inArray(this, parent.children);
            const that = this;
            ...
            res.done(data => {
                ...
                parent.children.splice(...[pos + 1, 0].concat(children));
    ...
    //            this.tree.debug("clearTimeout(%o)", this.tree.timer);
            ...
            const self = this; // required for closures
            switch (mode) {
            case "cancel":
                // Simply made sure that timer was cleared
                break;
            case "expand":
                this.tree.timer = setTimeout(() => {
    ...
    //        this.tree.debug("setTimeout(%s, %s): %s", mode, ms, this.tree.timer);
         ...
         * @returns {$.Promise}
         */
        scrollIntoView(effects, options) {
            ...
            const dfd = new $.Deferred();
            const that = this;
            const nodeHeight = $(this.span).height();
            const $container = $(opts.scrollParent);
            const topOfs = opts.scrollOfs.top || 0;
            const bottomOfs = opts.scrollOfs.bottom || 0;

            let // - topOfs - bottomOfs,
            containerHeight = $container.height();

            const scrollTop = $container.scrollTop();
            let $animateTarget = $container;
            const isParentWindow = $container[0] === window;
            const topNode = opts.topNode || null;
    ...
    //                dict.children = null;
        ...
        toggleClass(value, flag) {
            ...
            let i = 0;
            let wasAdded = false;
            const statusElem = this[this.tree.statusClassPropName];
            let curClasses = (` ${this.extraClasses || ""} `);
            ...
            while ( className = classNames[ i++ ] ) {
                hasClass = curClasses.includes(` ${className} `);
                ...
                if ( flag ) {
                    ...
                    while ( curClasses.includes(` ${className} `) ) {
        ...
        triggerModifyChild(operation, childNode, extra) {
            let data;
            const modifyChild = this.tree.options.modifyChild;
         ...
         * @since 2.4
         */
        visitAndLoad(fn, includeSelf, _recursion) {
            let dfd;
            let res;
            let loaders;
            const node = this;
    ...
    class Fancytree {
        constructor(widget) {
            ...
            if( this.options ) {
                if(  $.isFunction(this.options.lazyload ) && !$.isFunction(this.options.lazyLoad) ) {
                    this.options.lazyLoad = function(...args) {
                        FT.warn("The 'lazyload' event is deprecated since 2014-02-25. Use 'lazyLoad' (with uppercase L) instead.");
                        return widget.options.lazyload.apply(this, args);
                    };
                }
         ...
         * @returns {EventData}
         */
        _makeHookContext(obj, originalEvent, extra) {
            let ctx;
            let tree;
            if(obj.node !== undefined){
                ...
                tree = obj.tree;
                ctx = { node: obj, tree, widget: tree.widget, options: tree.widget.options, originalEvent };
            }else if(obj.widget){
                // obj is a Fancytree
                ctx = { node: null, tree: obj, widget: obj.widget, options: obj.widget.options, originalEvent };
            }else{
                $.error("invalid args");
            }
            if(extra){
         ...
         * @returns {any}
         */
        _callHook(funcName, contextObject, _extraArgs) {
            const ctx = this._makeHookContext(contextObject);
         ...
         */
        _requireExtension(name, required, before, message) {
            before = !!before;
            const thisName = this._local.name;
            const extList = this.options.extensions;
            const isBefore = $.inArray(name, extList) < $.inArray(thisName, extList);
            const isMissing = required && this.ext[name] == null;
            const badOrder = !isMissing && before != null && (before !== isBefore);
        ...
        applyPatch(patchList) {
            ...
            return $.when(...deferredList).promise();
        }
       ...
       */
         ...
         * @returns {FancytreeNode} matching node or null
         */
        findNextNode(match, startNode, visibleOnly) {
            let stopNode = null;
            const parentChildren = startNode.parent.children;
            let matchingNode = null;

            const walkVisible = (parent, idx, fn) => {
                let i;
                let grandParent;
                const parentChildren = parent.children;
                const siblingCount = parentChildren.length;
                const node = parentChildren[idx];
                // visit node itself
            ...
            walkVisible(startNode.parent, parentChildren.indexOf(startNode), node => {
         ...
         */
        generateFormElements(selected, active, opts = {}) {
            let nodeList;
            const selectedName = (typeof selected === "string") ? selected : `ft_${this._id}[]`;
            const activeName = (typeof active === "string") ? active : `ft_${this._id}_active`;
            const id = `fancytree_result_${this._id}`;
            let $result = $(`#${id}`);
            const stopOnParents = this.options.selectMode === 3 && opts.stopOnParents !== false;
    ...
    //            window.console.log("getNodeByKey(" + key + "): ", node.key);
        ...
        */
         ...
         * @returns {$.Promise}
         */
        loadKeyPath(keyPathList, callback, _rootNode) {
            let deferredList;
            let dfd;
            let i;
            let path;
            ...
            let root;
            let segList;
            const sep = this.options.keyPathSeparator;
            const self = this;
    ...
    //                    segList.unshift(key);
         ...
         */
        toDict(includeRoot, callback) {
            const res = this.rootNode.toDict(true, callback);
         ...
         * @see EventData
         */
        _triggerNodeEvent(type, node, originalEvent, extra) {
            //		this.debug("_trigger(" + type + "): '" + ctx.node.title + "'", ctx);
            const ctx = this._makeHookContext(node, originalEvent, extra);

            const res = this.widget._trigger(type, originalEvent, ctx);
        ...
        _triggerTreeEvent(type, originalEvent, extra) {
            //		this.debug("_trigger(" + type + ")", ctx);
            const ctx = this._makeHookContext(this, originalEvent, extra);
    ...
    //			this._callHook("nodeSetActive", ctx, true);
        ...
        nodeKeydown(ctx) {
            ...
            const whichChar = String.fromCharCode(which);
            const clean = !(event.altKey || event.ctrlKey || event.metaKey || event.shiftKey);
        ...
        nodeLoadChildren(ctx, source) {
            ...
            if(source.url){
                ...
                if(ajax.debugDelay){
                    ...
                    dfd = $.Deferred(dfd => {
                        setTimeout(() => {
                            $.ajax(ajax)
                                .done(function(...args) {	dfd.resolveWith(this, args); })
                                .fail(function(...args) {	dfd.rejectWith(this, args); });
                        }, delay);
                    });
                }else{
                    dfd = $.ajax(ajax);
                ...
                dfd.done(function (data, textStatus, jqXHR) {
                    ...
                    const errorObj = tree._makeHookContext(node, null, {
                        error: jqXHR,
                ...
                source.done(children => {
                    ...
                    if ( error === RECURSIVE_REQUEST_ERROR ) {
                        ...
                        ctxErr = tree._makeHookContext(node, null, {
                            error, // it can be jqXHR or any custom error
                            args: Array.prototype.slice.call(arguments),
            ...
            return $.when(source).done(children => {
                let metaData;
        ...
        /** [Not Implemented]  */
        nodeLoadKeyPath(ctx, keyPathList) {
        ...
        */
         ...
         */
        nodeRender(ctx, force, deep, collapsed, _recursive) {
    ...
    //				parent.ul.appendChild(node.li);
        ...
        nodeRenderStatus(ctx) {
            ...
            const //			nodeContainer = node[tree.nodeContainerAttrName],
            hasChildren = node.hasChildren();

            const isLastSib = node.isLastSibling();
            const aria = opts.aria;

            const // $ariaElem = aria ? $(node[tree.ariaPropName]) : null,
            $ariaElem = $(node.span).find(".fancytree-title");

            const cn = opts._classNames;
            const cnList = [];
            const statusElem = node[tree.statusClassPropName];
    ...
    //			tree.$container.attr("tabindex", "0");
         ...
         * @returns {$.Promise}
         */
        nodeSetActive(ctx, flag, callOpts = {}) {
            let subCtx;
            const node = ctx.node;
            const tree = ctx.tree;
            const opts = ctx.options;
            const noEvents = (callOpts.noEvents === true);
            const noFocus = (callOpts.noFocus === true);
            const isActive = (node === tree.activeNode);
         ...
         */
        nodeSetExpanded(ctx, flag, callOpts = {}) {
            let _afterLoad;
            let dfd;
            let i;
            let l;
            ...
            const tree = ctx.tree;
            const opts = ctx.options;
            let noAnimation = (callOpts.noAnimation === true);
            const noEvents = (callOpts.noEvents === true);
            ...
            dfd.done(() => {
                ...
                if( flag && opts.autoScroll && !noAnimation && lastChild ) {
                    ...
                    lastChild.scrollIntoView(true, {topNode: node}).always(() => {
                        if( !noEvents ) {
            ...
            _afterLoad = callback => {
                let isVisible;
                let isExpanded;
                const effect = opts.toggleEffect;
            ...
            if(flag && node.lazy && node.hasChildren() === undefined){
                ...
                node.load().done(() => {
                    ...
                    _afterLoad(() => { dfd.resolveWith(node); });
                }).fail(errMsg => {
                    _afterLoad(() => { dfd.rejectWith(node, [`load failed (${errMsg})`]); });
                });
    /*
    ...
    */
        ...
        nodeSetFocus(ctx, flag) {
            ...
            const // et = ctx.originalEvent && ctx.originalEvent.type,
    ...
    //			this._fixSelectionState(node);
        ...
        treeLoad(ctx, source) {
            ...
            let $ul;
            const tree = ctx.tree;
            const $container = ctx.widget.element;
            let dfd;

            const // calling context for root node
            ...
            dfd = this.nodeLoadChildren(rootCtx, source).done(() => {
                tree.render();
         ...
         * @param {boolean} [flag=true]
         */
        treeSetFocus(ctx, flag, callOpts) {
            ...
            if( flag !== this.hasFocus() ){
                ...
                if( flag ) {
                    ...
                    const thisTree = this;
                    setTimeout(() => { ensureTreeFocus(thisTree); }, 0);
                }
            }
        },
    ...
    //			this.tree[extName] = extension;
        ...
        _bind() {
            ...
            const // selstartEvent = ( $.support.selectstart ? "selectstart" : "mousedown" )
            ns = tree._ns;
            ...
            tree.$container.on(`focusin${ns} focusout${ns}`, event => {
                const node = FT.getNode(event);
                const flag = (event.type === "focusin");
            ...
            }).on(`selectstart${ns}`, "span.fancytree-title", event => {
                ...
                const // node may be null
                node = tree.focusNode;

                const ctx = tree._makeHookContext(node || tree, event);
                const prevPhase = tree.phase;
            ...
            }).on(`mousedown${ns} dblclick${ns}`, event => {
                // that.tree.debug("event(" + event + "): !");
                if(opts.disabled){
                    return true;
                }
                let ctx;
                const et = FT.getEventTarget(event);
                const node = et.node;
    ...
    $.extend($.ui.fancytree,
         ...
         * @param {any} [ctx]
         */
        debounce(timeout, fn, invokeAsap, ctx) {
            let timer;
         ...
         */
        fixPositionOptions(opts) {
            if( opts.offset || `${opts.my}${opts.at}`.includes("%") ) {
               $.error("expected new position syntax (but '%' is not supported)");
            }
            if( ! $.ui.fancytree.jquerySupports.positionMyOfs ) {
                const // parse 'left+3 center' into ['left+3 center', 'left', '+3', 'center', undefined]
                myParts = /(\w+)([+-]?\d+)?\s+(\w+)([+-]?\d+)?/.exec(opts.my);

                const atParts = /(\w+)([+-]?\d+)?\s+(\w+)([+-]?\d+)?/.exec(opts.at);

                const // convert to numbers
                dx = (myParts[2] ? (+myParts[2]) : 0) + (atParts[2] ? (+atParts[2]) : 0);

                const dy = (myParts[4] ? (+myParts[4]) : 0) + (atParts[4] ? (+atParts[4]) : 0);
        ...
        getEventTarget(event) {
            const tcn = event && event.target ? event.target.className : "";
            const res = {node: this.getNode(event.target), type: undefined};
         ...
         * @since 2.22
         */
        evalOption(optionName, node, nodeObject, treeOptions, defaultValue) {
            let ctx;
            let res;
            const tree = node.tree;
            const treeOpt = treeOptions[optionName];
            const nodeOpt = nodeObject[optionName];

            if( $.isFunction(treeOpt) ) {
                ctx = { node, tree, widget: tree.widget, options: tree.widget.options };
         ...
         */
        overrideMethod(instance, methodName, handler) {
            let prevSuper;
            const _super = instance[methodName] || $.noop;
        ...
        parseHtml($ul) {
            ...
            $children.each(function() {
                ...
                let lowerCaseAttr;
                const $li = $(this);
                const $liSpan = $li.find(">span:first", this);
                const $liA = $liSpan.length ? null : $li.find(">a:first");
                const d = { tooltip: null, data: {} };
