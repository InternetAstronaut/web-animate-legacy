...
const Potrace = () => {
  ...
  function processPath() {
    ...
    function interval(lambda, {x, y}, {x, y}) {
      const res = new Point();
    ...
    function cprod({x, y}, {x, y}, {x, y}, {x, y}) {
      let x1;
      let y1;
      let x2;
      let y2;
    ...
    function iprod1({x, y}, {x, y}, {x, y}, {x, y}) {
      let x1;
      let y1;
      let x2;
      let y2;
    ...
    function ddist({x, y}, {x, y}) {
      return Math.sqrt((x - x) * (x - x) + (y - y) * (y - y));
    }

    function bezier(t, {x, y}, {x, y}, {x, y}, {x, y}) {
      const s = 1 - t;
      const res = new Point();

      res.x = s*s*s*x + 3*(s*s*t)*x + 3*(t*t*s)*x + t*t*t*x;
      res.y = s*s*s*y + 3*(s*s*t)*y + 3*(t*t*s)*y + t*t*t*y;
...
const potraceImage = ({src}, callback, color) => {
...
const isElectronMode = window && window.process && window.process.type === "renderer";
