...
const SCWF = () => {
	...
	const SoundCloudWaveform = {
		...
		generate(src, options) {
	        ...
	        const rawData = src.split(",")[1]; // cut off extra filetype/etc data
	        const rawBuffer = Base64ArrayBuffer.decode(rawData);
		...
		extractBuffer(buffer) {
		    ...
		    for (let j = 0; j < sections; j += this.settings.bar_width) {
		        const scale = maxHeight / vals.max();
	    ...
	    drawBar(i, h) {
            ...
            const x = i + (w / 2);
            const y = this.settings.canvas.height/2 - h/2/1.5;
	    ...
	    generateImage() {
	    	const image = this.settings.canvas.toDataURL('image/png');
